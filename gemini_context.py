...
Helpers to call an LLM (Gemini) to:
...
This module uses Google's Gemini API via google.generativeai. If the package or
GEMINI_API_KEY are not available, functions fall back to conservative/empty values.
...
def _gemini_configured_model(model_name: Optional[str] = None):
    """
    Configure Gemini with GEMINI_API_KEY and return a model instance.
    ...
    try:
        ...
        name = (model_name or _DEFAULT_MODEL or "gemini-2.5-flash-lite").strip()
...
def _gemini_generate_text(prompt: str, model_name: Optional[str] = None, max_output_tokens: int = 1024) -> str:
    """
    Call Gemini with a single concatenated prompt (system + user content in one string).
    Returns the combined text from all candidate parts, or empty string on failure.
    """
    model = _gemini_configured_model(model_name)
    if not model:
        return ""
    try:
        resp = model.generate_content(prompt, generation_config={"max_output_tokens": max_output_tokens, "temperature": 0.0})
        if not resp:
            return ""
        text_parts: List[str] = []
